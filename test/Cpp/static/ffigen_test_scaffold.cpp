...
namespace ffigen_test_scaffold {
	...
	RustString test_string(const char* p) {
		...
		static FuncSignature funcPtr = reinterpret_cast<FuncSignature>(GetAddr("test_string"));
		return funcPtr(p);
	}
	
	RustString test_string_ref(const char* p) {
		typedef RustString (*FuncSignature)(const char*);
		static FuncSignature funcPtr = reinterpret_cast<FuncSignature>(GetAddr("test_string_ref"));
		return funcPtr(p);
	}
	
	RustString test_str_ref(const char* p) {
		typedef RustString (*FuncSignature)(const char*);
		static FuncSignature funcPtr = reinterpret_cast<FuncSignature>(GetAddr("test_str_ref"));
		return funcPtr(p);
	}
	...
	RustString test_multi_str(const char* p1, const char* p2, const char* p3) {
		...
		static FuncSignature funcPtr = reinterpret_cast<FuncSignature>(GetAddr("test_multi_str"));
		return funcPtr(p1, p2, p3);
	}
}
